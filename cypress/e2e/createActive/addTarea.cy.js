...
describe('Successfully add a new task to an asset', () => {
    ...
    it('should add a new task to an asset', () => {
        ...
        cy.get(addTarea.descripcion).eq(0).type('Verificar temperatura interna y ajustar termostato si es necesario.')
        ...
        cy.get(addTarea.btn_evento_prueba).should('be.visible').should('exist')
        cy.get(addTarea.btn_evento_prueba).click()
        ...
        cy.get(addTarea.icono_flecha_izquierda).first().click()
        cy.wait(5000)
        ...
        cy.get(addTarea.popup_activos_vinculado).should('be.visible').should('exist')
    })
})
